Aby uruchomić projekt należy wykonać następujące kroki:

1. Należy otworzyć folder Projekt_WS w wierszu polecenia.


2. Należy zainicjować bazę danych w MongoDB

- w wierszu poleceń należy wpisać poniższą komendę

	node InitDatabase.js

3. Należy uruchomić projekt z folderu ProjectMongoDBReact w środowisku Visual Studio

4. By przetestować aplikację, należy się zarejestrować za pomocą dowolnego adresu e-mail i hasła.
Stworzone konto będzie kontem klienta hotelu.

	