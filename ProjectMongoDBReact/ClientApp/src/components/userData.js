...
export class userData extends Component {
  ...
  constructor(props) {
    super(props);
      this.state = { email: props.match.params.email, name: '',rezerwacje: [], loading: true, show: false };
  ...
  render() {
    let contents = this.state.loading
        ...
        : userData.renderRezerwacjeTable(this.state.rezerwacje, this.state.show);

    return (
        <div>
            <h1 id="tabelLabel" >Lista rezerwacji {this.state.email}</h1>
            <br /><br />
  ...
  async populateUserData() {
      ...
      const response = await fetch('rezerwacja/GetRezerwacjeDetails/' + this.state.name + '/' + this.state.email, {
